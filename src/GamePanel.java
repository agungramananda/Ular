...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void checkCollisions(){
        //check apakah kepala tabrakan dengan badan
        for (int i = bodyParts;i>0;i--){
            if ((x[0] == x[i])&&(y[0]==y[i])){
                running = false;
            }
        }
        //check apakah kepala tabrakan dengan tembok
        //kiri
        if(x[0]<0){
            running = false;
        }
    ...
    public void gameOver(Graphics g){
        ...
        g.drawString("Game Over",(SCREEN_WIDHT - metrics1.stringWidth("Game Over"))/2, SCREEN_HEIGHT/2);
        //Score
        g.setColor(Color.red);
        g.setFont(new Font("Ink Free",Font.BOLD,40));
        FontMetrics metrics2 = getFontMetrics(g.getFont());
        g.drawString("Score : "+applesEaten,(SCREEN_WIDHT - metrics2.stringWidth("Score : "+applesEaten))/2, g.getFont().getSize());
    }
    @Override
